  ...
  if (Array.isArray(raw)) {
    for (const item of raw) {
      if (item && typeof item === 'object') {
        ...
        list.push({ id: item, name: String(item), parent_id: null })
      }
    }
  } else if (typeof raw === 'object' && raw) {
    for (const [idKey, nameVal] of Object.entries(raw)) {
      const idNum = /^\d+$/.test(idKey) ? Number(idKey) : idKey
      list.push({ id: idNum, name: String(nameVal ?? ''), parent_id: null })
    }
  }
  list.unshift({ id: null, name: 'Nenhuma (raiz)', parent_id: null })
  return list
})
...
function submit() {
  form.post(route('categories.store'), {
    onSuccess: () => {
      ...
      router.reload({ only: ['parentOptions'], preserveScroll: true })
    }
  })
}
</script>
...
<template>
  ...
  <AppLayout title="Criar Categoria">
    <template #header>
      <h2 class="font-semibold text-xl text-gray-800 leading-tight">Criar Categoria</h2>
    </template>

    <FormSection @submit.prevent="submit" class="p-6 space-y-6">
      <template #form>
        ...
        <div class="w-full sm:grid lg:grid-cols-2 sm:grid-cols-1 gap-2">
          ...
          <div v-if="cascade.length" class="col-span-2 space-y-2">
            ...
            <ul class="relative">
              <li v-for="row in cascade" :key="row.item.id"
